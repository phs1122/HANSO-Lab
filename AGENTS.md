...
- `.hanso` 컨테이너의 공식 기준은 [`../hanso-dsp/docs/HANSO_CONTAINER_FORMAT.md`](../hanso-dsp/docs/HANSO_CONTAINER_FORMAT.md)이다.
- `.hanso` export/container/schema 작업 전에는 먼저 [`docs/HANSO_CONTAINER_REFERENCE.md`](docs/HANSO_CONTAINER_REFERENCE.md)와 공식 기준 문서를 함께 읽는다.
...
- `Source/Serialization/HansoSerializer.*`, `Source/Model/HansoPackage.*`, `captureWorkflow`, `cabProfile`, `captureData`, `modelData`, `dspCore.captureModel`, chunk id/role/mediaType/encoding을 바꾸는 작업은 같은 변경에서 공통 명세를 갱신한다.
- `.hanso` producer 변경 후 루트에서 `scripts/check-hanso-container-spec.sh`를 실행하고, 관련 Lab regression test도 실행한다.
...
- git 관리는 사용자 승인 아래 수행한다.
- commit, push, branch 등 git 상태를 바꾸는 작업은 사용자가 요청했거나 명시적으로 승인한 경우에만 한다.
- 승인 없이 작업하는 동안에는 로컬 파일 작업과 빌드/테스트 확인만 수행한다.
- 커밋은 작고 응집되게, 의미 있는 메시지로 작성하고 생성물/캐시는 커밋하지 않는다.
