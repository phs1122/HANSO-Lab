...
- 패키지에 `cabProfile.micMatrix`가 있으면 **Mic 콤보(Original/Dynamic/Ribbon/Condenser)**로 마이크 전환을 프리뷰할 수 있다. 선택한 마이크와 실측 마이크의 tone profile 차이를 IR 프리뷰 출력 위에 6밴드 EQ로 적용하며, Original은 EQ 없이 실측 IR 그대로다.
